...
UI/UX について気づいたことを、あとで記事にまとめられる構造化された観察メモとして蓄積する。

「あの操作がイラッとした」程度の雑な入力で成立することを設計要件とし、評価軸を先に押し付けず、掘り下げの質問は一度に1つまでに制限する。1回の出力は記事ではなく観察メモで、溜まってから作品単位・軸単位で記事にまとめる。

- `references/axes.md` — 観察軸の定義。軸の一覧・定義・個数を持つ唯一の場所
- `templates/observation.md` — 観察メモ1件の形式
- `templates/article-draft.md` — 記事下書きの形式

観察メモ本体と作品名・アプリ名はこのリポジトリには置かず、公開しない別リポジトリで管理する。軸は製品に依存しない原則として書く。
